...
/** \brief Crea una nueva lista filtrada con los elementos que cumplen las condiciones que se veran en pFunc
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return int Retorna  (-1) Error: si el puntero a la listas es NULL
                                ( 0) Si ok
...
/** \brief Transforma una lista en una nueva con los elementos que cumplen ciertas condiciones que se veran en pFunc
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return int Retorna  (-1) Error: si el puntero a la listas es NULL
                                ( 0) Si ok
...
LinkedList* ll_map(LinkedList* this, void* (*pFunc)(void* element)) {
	LinkedList* mapedList = NULL;
	int longList;

	if(this != NULL && pFunc != NULL) {
		mapedList = ll_newLinkedList();
		longList = ll_len(this);

		for(int i = 0; i < longList; i++) {
			void* element = ll_get(this, i);
			ll_add(mapedList, pFunc(element));
		}
	}
